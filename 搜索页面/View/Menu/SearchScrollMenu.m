...
#define tagIndex 20170510

@interface SearchScrollMenu()
...
@implementation SearchScrollMenu
...
- (void)setUpMenuBtns{
    ...
    for (int i = 0; i < self.menuArray.count; i++) {
        ...
        CGFloat menuBtnX = 50.f;
        if (self.scrollView.subviews.count > 0) {
            //非第一个
            menuBtnX = CGRectGetMaxX([self.scrollView.subviews lastObject].frame) + 50;
        }
    ...
    if (self.scrollView.subviews.count > 0) {
        self.scrollView.contentSize = CGSizeMake(CGRectGetMaxX([self.scrollView.subviews lastObject].frame) + 50.f, 0);
    }
